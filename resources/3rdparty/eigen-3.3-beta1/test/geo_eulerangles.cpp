...
void verify_euler(const Matrix<Scalar,3,1>& ea, int i, int j, int k)
{
  ...
  if( (i!=k || ea[1]!=0) && (i==k || !internal::isApprox(abs(ea[1]),Scalar(STORMEIGEN_PI/2),test_precision<Scalar>())) ) 
    VERIFY((ea-eabis).norm() <= test_precision<Scalar>());
  
  // approx_or_less_than does not work for 0
  VERIFY(0 < eabis[0] || test_isMuchSmallerThan(eabis[0], Scalar(1)));
  VERIFY_IS_APPROX_OR_LESS_THAN(eabis[0], Scalar(STORMEIGEN_PI));
  VERIFY_IS_APPROX_OR_LESS_THAN(-Scalar(STORMEIGEN_PI), eabis[1]);
  VERIFY_IS_APPROX_OR_LESS_THAN(eabis[1], Scalar(STORMEIGEN_PI));
  VERIFY_IS_APPROX_OR_LESS_THAN(-Scalar(STORMEIGEN_PI), eabis[2]);
  VERIFY_IS_APPROX_OR_LESS_THAN(eabis[2], Scalar(STORMEIGEN_PI));
}
...
template<typename Scalar> void eulerangles()
{
  ...
  ea = (Array3::Random() + Array3(1,0,0))*Scalar(STORMEIGEN_PI)*Array3(0.5,1,1);
  check_all_var(ea);
  
  ea[2] = ea[0] = internal::random<Scalar>(0,Scalar(STORMEIGEN_PI));
  check_all_var(ea);
  
  ea[0] = ea[1] = internal::random<Scalar>(0,Scalar(STORMEIGEN_PI));
  check_all_var(ea);
