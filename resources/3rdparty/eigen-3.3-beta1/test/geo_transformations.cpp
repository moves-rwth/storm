...
template<typename Scalar, int Mode, int Options> void transformations()
{
  ...
  */
  ...
  VERIFY_IS_APPROX(-v0, AngleAxisx(Scalar(STORMEIGEN_PI), v0.unitOrthogonal()) * v0);
  ...
  while (abs(a)<Scalar(0.1))
    a = internal::random<Scalar>(-Scalar(0.4)*Scalar(STORMEIGEN_PI), Scalar(0.4)*Scalar(STORMEIGEN_PI));
  q1 = AngleAxisx(a, v0.normalized());
  Transform3 t0, t1, t2;
  ...
  Scalar a3 = internal::random<Scalar>(-Scalar(STORMEIGEN_PI), Scalar(STORMEIGEN_PI));
  ...
  for(int k=0; k<100; ++k)
  {
    ...
    VERIFY( rot2.smallestPositiveAngle() <= Scalar(2)*Scalar(STORMEIGEN_PI) );
    VERIFY_IS_APPROX( angleToVec(rot2.smallestPositiveAngle()), angleToVec(rot2.angle()) );
    
    VERIFY( rot2.smallestAngle() >= -Scalar(STORMEIGEN_PI) );
    VERIFY( rot2.smallestAngle() <=  Scalar(STORMEIGEN_PI) );
  ...
  if(std::cos(s0)>0)
    ...
    VERIFY_IS_APPROX(Scalar(STORMEIGEN_PI), (R0.slerp(0.5, R0.inverse())).smallestPositiveAngle());
  ...
  VERIFY(l<=STORMEIGEN_PI*(Scalar(1)+NumTraits<Scalar>::epsilon()*Scalar(path_steps/2)));
