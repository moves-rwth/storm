...
void ctms_decompositions()
{
  ...
  typedef StormEigen::Matrix<Scalar,
                        StormEigen::Dynamic, StormEigen::Dynamic,
                        0,
                        maxSize, maxSize> Matrix;

  typedef StormEigen::Matrix<Scalar,
                        StormEigen::Dynamic, 1,
                        0,
                        maxSize, 1> Vector;

  typedef StormEigen::Matrix<std::complex<Scalar>,
                        StormEigen::Dynamic, StormEigen::Dynamic,
                        0,
  ...
  // Cholesky module
  StormEigen::LLT<Matrix>  LLT;  LLT.compute(A);
  X = LLT.solve(B);
  x = LLT.solve(b);
  StormEigen::LDLT<Matrix> LDLT; LDLT.compute(A);
  X = LDLT.solve(B);
  x = LDLT.solve(b);
  ...
  StormEigen::EigenSolver<Matrix>                    eigSolver;         eigSolver.compute(A);
  StormEigen::SelfAdjointEigenSolver<Matrix>         saEigSolver(size); saEigSolver.compute(saA);
  StormEigen::Tridiagonalization<Matrix>             tridiag;           tridiag.compute(saA);

  // LU module
  StormEigen::PartialPivLU<Matrix> ppLU; ppLU.compute(A);
  X = ppLU.solve(B);
  x = ppLU.solve(b);
  StormEigen::FullPivLU<Matrix>    fpLU; fpLU.compute(A);
  X = fpLU.solve(B);
  x = fpLU.solve(b);

  // QR module
  StormEigen::HouseholderQR<Matrix>        hQR;  hQR.compute(A);
  X = hQR.solve(B);
  x = hQR.solve(b);
  StormEigen::ColPivHouseholderQR<Matrix>  cpQR; cpQR.compute(A);
  X = cpQR.solve(B);
  x = cpQR.solve(b);
  StormEigen::FullPivHouseholderQR<Matrix> fpQR; fpQR.compute(A);
...
template<typename MatrixType> void test_reference(const MatrixType& m) {
  ...
  typedef StormEigen::Matrix<Scalar, StormEigen::Dynamic, Eigen::Dynamic, Flag         > MatrixX;
  typedef StormEigen::Matrix<Scalar, StormEigen::Dynamic, Eigen::Dynamic, TransposeFlag> MatrixXT;
  // Dynamic reference:
  typedef StormEigen::Ref<const MatrixX  > Ref;
  typedef StormEigen::Ref<const MatrixXT > RefT;
  ...
  typedef StormEigen::Ref<const MatrixX, Aligned> RefAligned;
