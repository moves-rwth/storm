...
void test_product_small()
{
  ...
  
  {
    ...
    StormEigen::Matrix<double, 1, 1> A;  A.setRandom();
    StormEigen::Matrix<double, 18, 1> B; B.setRandom();
    StormEigen::Matrix<double, 1, 18> C; C.setRandom();
    VERIFY_IS_APPROX(B * A.inverse(), B * A.inverse()[0]);
    VERIFY_IS_APPROX(A.inverse() * C, A.inverse()[0] * C);
  }

  {
    StormEigen::Matrix<double, 10, 10> A, B, C;
    A.setRandom();
    C = A;
    for(int k=0; k<79; ++k)
      C = C * A;
