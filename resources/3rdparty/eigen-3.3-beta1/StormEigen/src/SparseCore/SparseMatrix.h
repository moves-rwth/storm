...
#ifndef STORMEIGEN_SPARSEMATRIX_H
#define STORMEIGEN_SPARSEMATRIX_H
...
/** \ingroup SparseCore_Module
  ...
  * \ref TopicCustomizingEigen by defining the preprocessor symbol \c STORMEIGEN_SPARSEMATRIX_PLUGIN.
  */
  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _Index> >
{
  ...
  public:
    ...
    using Base::nonZeros;
    STORMEIGEN_SPARSE_PUBLIC_INTERFACE(SparseMatrix)
    using Base::operator+=;
    using Base::operator-=;
  ...
  public:
    ...
    #if (!STORMEIGEN_COMP_MSVC) || (STORMEIGEN_COMP_MSVC>=1500) // MSVC 2005 fails to compile with this typename
        typename
    #endif
        SizesType::value_type())
    {
      STORMEIGEN_UNUSED_VARIABLE(enableif);
  ...
  public:
    ...
    inline SparseMatrix(const SparseMatrixBase<OtherDerived>& other)
      : m_outerSize(0), m_innerSize(0), m_outerIndex(0), m_innerNonZeros(0)
    {
      STORMEIGEN_STATIC_ASSERT((internal::is_same<Scalar, typename OtherDerived::Scalar>::value),
      ...
      {
        #ifdef STORMEIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
          STORMEIGEN_SPARSE_CREATE_TEMPORARY_PLUGIN
        #endif
    ...
    {
      //STORMEIGEN_DBG_SPARSE(std::cout << "SparseMatrix:: swap\n");
    ...
    template<typename OtherDerived>
    STORMEIGEN_DONT_INLINE SparseMatrix& operator=(const SparseMatrixBase<OtherDerived>& other);

    friend std::ostream & operator << (std::ostream & s, const SparseMatrix& m)
    {
      STORMEIGEN_DBG_SPARSE(
        s << "Nonzero entries:\n";
        if(m.isCompressed())
...
#   ifdef STORMEIGEN_SPARSEMATRIX_PLUGIN
#     include STORMEIGEN_SPARSEMATRIX_PLUGIN
#   endif
...
protected:
    ...
    STORMEIGEN_DONT_INLINE Scalar& insertCompressed(Index row, Index col);
    ...
    /** \internal
      * \sa insert(Index,Index) */
    STORMEIGEN_DONT_INLINE Scalar& insertUncompressed(Index row, Index col);

public:
    /** \internal
      * \sa insert(Index,Index) */
    STORMEIGEN_STRONG_INLINE Scalar& insertBackUncompressed(Index row, Index col)
    {
      const Index outer = IsRowMajor ? row : col;
      const Index inner = IsRowMajor ? col : row;
      ...
      m_data.index(p) = convert_index(inner);
        return (m_data.value(p) = storm::utility::zero<Scalar>());
    }

private:
  static void check_template_parameters()
  {
    STORMEIGEN_STATIC_ASSERT(NumTraits<StorageIndex>::IsSigned,THE_INDEX_TYPE_MUST_BE_A_SIGNED_TYPE);
    STORMEIGEN_STATIC_ASSERT((Options&(ColMajor|RowMajor))==Options,INVALID_MATRIX_TEMPLATE_PARAMETERS);
  }
...
template<typename Scalar, int _Options, typename _Index>
template<typename OtherDerived>
STORMEIGEN_DONT_INLINE SparseMatrix<Scalar,_Options,_Index>& SparseMatrix<Scalar,_Options,_Index>::operator=(const SparseMatrixBase<OtherDerived>& other)
{
  STORMEIGEN_STATIC_ASSERT((internal::is_same<Scalar, typename OtherDerived::Scalar>::value),
        YOU_MIXED_DIFFERENT_NUMERIC_TYPES__YOU_NEED_TO_USE_THE_CAST_METHOD_OF_MATRIXBASE_TO_CAST_NUMERIC_TYPES_EXPLICITLY)
  ...
  if (needToTranspose)
  {
    #ifdef STORMEIGEN_SPARSE_TRANSPOSED_COPY_PLUGIN
      STORMEIGEN_SPARSE_TRANSPOSED_COPY_PLUGIN
    #endif
...
template<typename _Scalar, int _Options, typename _Index>
STORMEIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_Index>::Scalar& SparseMatrix<_Scalar,_Options,_Index>::insertUncompressed(Index row, Index col)
{
  eigen_assert(!isCompressed());
...
template<typename _Scalar, int _Options, typename _Index>
STORMEIGEN_DONT_INLINE typename SparseMatrix<_Scalar,_Options,_Index>::Scalar& SparseMatrix<_Scalar,_Options,_Index>::insertCompressed(Index row, Index col)
{
  eigen_assert(isCompressed());
...
#endif // STORMEIGEN_SPARSEMATRIX_H
