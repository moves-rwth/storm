...
#ifndef STORMEIGEN_GENERAL_MATRIX_VECTOR_H
#define STORMEIGEN_GENERAL_MATRIX_VECTOR_H
 ...
 *       // this re-alignment is done by the palign function implemented for SSE in StormEigen/src/Core/arch/SSE/PacketMath.h
 *   - otherwise,
...
STORMEIGEN_DONT_INLINE static void run(
...
template<typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar, typename RhsMapper, bool ConjugateRhs, int Version>
STORMEIGEN_DONT_INLINE void general_matrix_vector_product<Index,LhsScalar,LhsMapper,ColMajor,ConjugateLhs,RhsScalar,RhsMapper,ConjugateRhs,Version>::run(
  Index rows, Index cols,
  const LhsMapper& lhs,
  const RhsMapper& rhs,
        ResScalar* res, Index resIncr,
  RhsScalar alpha)
{
  STORMEIGEN_UNUSED_VARIABLE(resIncr);
  eigen_internal_assert(resIncr==1);
  #ifdef _STORMEIGEN_ACCUMULATE_PACKETS
  #error _STORMEIGEN_ACCUMULATE_PACKETS has already been defined
  #endif
  #define _STORMEIGEN_ACCUMULATE_PACKETS(Alignment0,Alignment13,Alignment2) \
    pstore(&res[j], \
...
//  const Index PeelAlignedMask = ResPacketSize*peels-1;
        ...
        switch(alignmentPattern)
        {
          case AllAligned:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _STORMEIGEN_ACCUMULATE_PACKETS(Aligned,Aligned,Aligned);
            break;
          case EvenAligned:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _STORMEIGEN_ACCUMULATE_PACKETS(Aligned,Unaligned,Aligned);
            break;
          case FirstAligned:
          {
            ...
            for (; j<alignedSize; j+=ResPacketSize)
              _STORMEIGEN_ACCUMULATE_PACKETS(Aligned,Unaligned,Unaligned);
            break;
          }
          default:
            for (Index j = alignedStart; j<alignedSize; j+=ResPacketSize)
              _STORMEIGEN_ACCUMULATE_PACKETS(Unaligned,Unaligned,Unaligned);
            break;
        }
      }
  ...
  #undef _STORMEIGEN_ACCUMULATE_PACKETS
}
...
template<typename Index, typename LhsScalar, typename LhsMapper, bool ConjugateLhs, typename RhsScalar, typename RhsMapper, bool ConjugateRhs, int Version>
STORMEIGEN_DONT_INLINE void general_matrix_vector_product<Index,LhsScalar,LhsMapper,RowMajor,ConjugateLhs,RhsScalar,RhsMapper,ConjugateRhs,Version>::run(
  Index rows, Index cols,
  const LhsMapper& lhs,
  const RhsMapper& rhs,
  ResScalar* res, Index resIncr,
  ResScalar alpha)
{
  ...
  {
    // FIXME: what is the purpose of this STORMEIGEN_ALIGN_DEFAULT ??
    STORMEIGEN_ALIGN_MAX ResScalar tmp0 = ResScalar(0);
...
#endif // STORMEIGEN_GENERAL_MATRIX_VECTOR_H
