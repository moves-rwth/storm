...
#ifndef STORMEIGEN_COLAMD_H
#define STORMEIGEN_COLAMD_H
...
static bool colamd(IndexType n_row, IndexType n_col, IndexType Alen, IndexType *A, IndexType *p, double knobs[COLAMD_KNOBS], IndexType stats[COLAMD_STATS])
{
  ...
  if (!StormEigen::internal::init_rows_cols (n_row, n_col, Row, Col, A, p, stats))
  {
  ...
  StormEigen::internal::init_scoring (n_row, n_col, Row, Col, A, p, knobs,
		&n_row2, &n_col2, &max_deg) ;
  
  /* === Order the supercolumns =========================================== */
  
  ngarbage = StormEigen::internal::find_ordering (n_row, n_col, Alen, Row, Col, A, p,
			    n_col2, max_deg, 2*nnz) ;
  
  /* === Order the non-principal columns ================================== */
  
  StormEigen::internal::order_children (n_col, Col, p) ;
    ...
    )
{
  ...
  tag_mark = StormEigen::internal::clear_mark (n_row, Row) ;
  min_score = 0 ;
  ngarbage = 0 ;
  ...
  for (k = 0 ; k < n_col2 ; /* 'k' is incremented below */)
  {
    ...
    {
      pfree = StormEigen::internal::garbage_collection (n_row, n_col, Row, Col, A, &A [pfree]) ;
      ngarbage++ ;
	...
	*new_cp++ = row ;
    ...
    StormEigen::internal::detect_super_cols (Col, A, head, pivot_row_start, pivot_row_length) ;
