...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_CONVERSION_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_CONVERSION_H
...
struct PacketConverter {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
...
struct PacketConverter<TensorEvaluator, SrcPacket, TgtPacket, 1, 2> {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TgtPacket packet(Index index) const {
    ...
    if (m_impl.data() && (index + SrcPacketSize < m_maxIndex)) {
      ...
      STORMEIGEN_ALIGN_MAX typename internal::unpacket_traits<TgtPacket>::type values[TgtPacketSize];
...
class TensorConversionOp : public TensorBase<TensorConversionOp<TargetType, XprType>, ReadOnlyAccessors>
{
  public:
    ...
    STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorConversionOp(const XprType& xpr)
        : m_xpr(xpr) {}
...
struct TensorEvaluator<const TensorConversionOp<TargetType, ArgType>, Device>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE const Dimensions& dimensions() const { return m_impl.dimensions(); }
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_CONVERSION_H
