...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_CONTRACTION_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_CONTRACTION_H
...
class SimpleTensorContractionMapper {
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE void prefetch(Index /*i*/) { }

  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Scalar operator()(Index row) const {
    // column major assumption
    return operator()(row, 0);
  }

  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Scalar operator()(Index row, Index col) const {
    return m_tensor.coeff(computeIndex(row, col));
  }

  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Index computeIndex(Index row, Index col) const {
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE IndexPair<Index> computeIndexPair(Index row, Index col, const Index distance) const {
    const bool left = (side == Lhs);
  ...
  class BaseTensorContractionMapper : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, packet_size, inner_dim_contiguous>
{
 public:
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Packet loadPacket(Index i, Index j) const {
    // whole method makes column major assumption

    // don't need to add offsets for now (because operator handles that)
    // current code assumes packet size must be a multiple of 2
    STORMEIGEN_STATIC_ASSERT(packet_size % 2 == 0, YOU_MADE_A_PROGRAMMING_MISTAKE);
    ...
    STORMEIGEN_ALIGN_MAX Scalar data[packet_size];
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE HalfPacket loadHalfPacket(Index i, Index j) const {
    // whole method makes column major assumption
    ...
    STORMEIGEN_ALIGN_MAX Scalar data[half_packet_size];
...
class BaseTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, 1, inner_dim_contiguous, inner_dim_reordered, Alignment> : public SimpleTensorContractionMapper<Scalar, Index, side, Tensor, nocontract_t, contract_t, 1, inner_dim_contiguous>
{
 public:
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Packet loadPacket(Index i, Index j) const {
    STORMEIGEN_ALIGN_MAX Scalar data[1];
    data[0] = this->m_tensor.coeff(this->computeIndex(i, j));
    return pload<typename packet_traits<Scalar>::type>(data);
  }
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE Packet loadHalfPacket(Index i, Index j) const {
    return loadPacket(i, j);
  }
};
...
class TensorContractionSubMapper {
 public:
  ...
  STORMEIGEN_DEVICE_FUNC TensorContractionSubMapper(const ParentMapper& base_mapper, Index vert_offset, Index horiz_offset)
      : m_base_mapper(base_mapper), m_vert_offset(vert_offset), m_horiz_offset(horiz_offset) { }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE Scalar operator()(Index i) const {
    return m_base_mapper(i + m_vert_offset, m_horiz_offset);
  }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE Scalar operator()(Index i, Index j) const {
    return m_base_mapper(i + m_vert_offset, j + m_horiz_offset);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE Packet loadPacket(Index i) const {
    return m_base_mapper.loadPacket(i + m_vert_offset, m_horiz_offset);
  }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE Packet loadPacket(Index i, Index j) const {
   return m_base_mapper.loadPacket(i + m_vert_offset, j + m_horiz_offset);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE HalfPacket loadHalfPacket(Index i) const {
    return m_base_mapper.loadHalfPacket(i + m_vert_offset, m_horiz_offset);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE void storePacket(Index i, Packet p) const {
    m_base_mapper.storePacket(i + m_vert_offset, m_horiz_offset, p);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE LinearMapper getLinearMapper(Index i, Index j) const {
    return LinearMapper(m_base_mapper, i + m_vert_offset, j + m_horiz_offset);
  }

  template <typename PacketT, int AlignmentType>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE PacketT load(Index i) const {
    STORMEIGEN_STATIC_ASSERT((internal::is_same<PacketT, Packet>::value), YOU_MADE_A_PROGRAMMING_MISTAKE);
    STORMEIGEN_STATIC_ASSERT((AlignmentType == Aligned || Alignment == Unaligned), YOU_MADE_A_PROGRAMMING_MISTAKE);
    return loadPacket(i);
  }

  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC bool aligned(Index) const {
    return false;
  }
...
class TensorContractionInputMapper
 ...
 public:
  ...
  STORMEIGEN_DEVICE_FUNC TensorContractionInputMapper(const Tensor& tensor,
                               const nocontract_t& nocontract_strides,
                               const nocontract_t& ij_strides,
                               const contract_t& contract_strides,
                               const contract_t& k_strides)
  ...
  STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE SubMapper getSubMapper(Index i, Index j) const {
    return SubMapper(*this, i, j);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE VectorMapper getVectorMapper(Index i, Index j) const {
...
class TensorContractionOp : public TensorBase<TensorContractionOp<Indices, LhsXprType, RhsXprType>, ReadOnlyAccessors>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorContractionOp(
...
struct TensorContractionEvaluatorBase
{
  ...
  TensorContractionEvaluatorBase(const XprType& op, const Device& device)
    ...
    m_rightImpl(choose(Cond<static_cast<int>(Layout) == static_cast<int>(ColMajor)>(),
        ...
        m_device(device),
        m_result(NULL) {
    STORMEIGEN_STATIC_ASSERT((static_cast<int>(TensorEvaluator<LeftArgType, Device>::Layout) ==
  ...
  STORMEIGEN_DEVICE_FUNC void evalTo(Scalar* buffer) const {
  ...
  STORMEIGEN_DEVICE_FUNC PacketReturnType packet(Index index) const {
...
struct TensorEvaluator<const TensorContractionOp<Indices, LeftArgType, RightArgType>, Device> :
  ...
  STORMEIGEN_DEVICE_FUNC TensorEvaluator(const XprType& op, const Device& device) :
      Base(op, device) { }
  ...
  template <bool lhs_inner_dim_contiguous, bool rhs_inner_dim_contiguous, bool rhs_inner_dim_reordered, int Alignment>
  STORMEIGEN_DEVICE_FUNC void evalGemm(Scalar* buffer) const {
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_CONTRACTION_H
