...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_DIMENSIONS_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_DIMENSIONS_H
...
template <typename Index> struct IndexPair {
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE IndexPair() : first(0), second(0) { }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE IndexPair(Index f, Index s) : first(f), second(s) { }
  Index first;
  Index second;
};
...
{
  template <typename Dimensions> STORMEIGEN_DEVICE_FUNC
...
struct Sizes : internal::numeric_list<std::ptrdiff_t, Indices...> {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t rank() const {
    return Base::count;
  }

  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t TotalSize() {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t operator[] (const std::size_t index) const {
    return internal::fixed_size_tensor_index_extraction_helper<std::ptrdiff_t, Base::count>::run(index, *this);
  }

  template <typename DenseIndex> STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE
...
namespace internal {
template <typename std::ptrdiff_t... Indices>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t array_prod(const Sizes<Indices...>&) {
...
template <std::size_t V1=0, std::size_t V2=0, std::size_t V3=0, std::size_t V4=0, std::size_t V5=0> struct Sizes {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE size_t rank() const {
    return count;
  }

  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE size_t TotalSize() {
...
#else
  STORMEIGEN_DEVICE_FUNC explicit Sizes(const DenseIndex) {
  }
  STORMEIGEN_DEVICE_FUNC explicit Sizes(const DenseIndex, const DenseIndex) {
  }
  STORMEIGEN_DEVICE_FUNC explicit Sizes(const DenseIndex, const DenseIndex, const DenseIndex) {
  }
  STORMEIGEN_DEVICE_FUNC explicit Sizes(const DenseIndex, const DenseIndex, const DenseIndex, const DenseIndex) {
  }
  STORMEIGEN_DEVICE_FUNC explicit Sizes(const DenseIndex, const DenseIndex, const DenseIndex, const DenseIndex, const DenseIndex) {
  }
#endif

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE DenseIndex operator[] (const int index) const {
    switch (index) {
      case 0:
...
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::size_t array_prod(const Sizes<V1, V2, V3, V4, V5>&) {
...
{
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE
...
struct DSizes : array<DenseIndex, NumDims> {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE DenseIndex TotalSize() const {
    return internal::array_prod(*static_cast<const Base*>(this));
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE DSizes() {
    for (int i = 0 ; i < NumDims; ++i) {
      (*this)[i] = 0;
    }
  }
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const array<DenseIndex, NumDims>& a) : Base(a) { }

#ifdef STORMEIGEN_HAS_VARIADIC_TEMPLATES
  template<typename... IndexTypes> STORMEIGEN_DEVICE_FUNC
  STORMEIGEN_STRONG_INLINE explicit DSizes(DenseIndex firstDimension, IndexTypes... otherDimensions) {
    STORMEIGEN_STATIC_ASSERT(sizeof...(otherDimensions) + 1 == NumDims, YOU_MADE_A_PROGRAMMING_MISTAKE)
    (*this) = array<DenseIndex, NumDims>{{firstDimension, otherDimensions...}};
  }
#else
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const DenseIndex i0) {
    eigen_assert(NumDims == 1);
    (*this)[0] = i0;
  }
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const DenseIndex i0, const DenseIndex i1) {
    eigen_assert(NumDims == 2);
    (*this)[0] = i0;
    (*this)[1] = i1;
  }
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const DenseIndex i0, const DenseIndex i1, const DenseIndex i2) {
    eigen_assert(NumDims == 3);
    (*this)[0] = i0;
    (*this)[1] = i1;
    (*this)[2] = i2;
  }
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const DenseIndex i0, const DenseIndex i1, const DenseIndex i2, const DenseIndex i3) {
  ...
  STORMEIGEN_DEVICE_FUNC explicit DSizes(const DenseIndex i0, const DenseIndex i1, const DenseIndex i2, const DenseIndex i3, const DenseIndex i4) {
  ...
  STORMEIGEN_DEVICE_FUNC DSizes& operator = (const array<DenseIndex, NumDims>& other) {
    *static_cast<Base*>(this) = other;
    return *this;
  }

  // A constexpr would be so much better here
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE DenseIndex IndexOfColMajor(const array<DenseIndex, NumDims>& indices) const {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE DenseIndex IndexOfRowMajor(const array<DenseIndex, NumDims>& indices) const {
...
template <std::ptrdiff_t n, typename std::ptrdiff_t... Indices> STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t array_get(const Sizes<Indices...>&) {
  return get<n, internal::numeric_list<std::size_t, Indices...> >::value;
}
template <std::ptrdiff_t n> STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::ptrdiff_t array_get(const Sizes<>&) {
...
template <std::size_t n, std::size_t V1, std::size_t V2, std::size_t V3, std::size_t V4, std::size_t V5> STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE std::size_t array_get(const Sizes<V1,V2,V3,V4,V5>&) {
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_DIMENSIONS_H
