...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_REF_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_REF_H
...
class TensorLazyBaseEvaluator {
 public:
  ...
  STORMEIGEN_DEVICE_FUNC virtual const Dimensions& dimensions() const = 0;
  STORMEIGEN_DEVICE_FUNC virtual const Scalar* data() const = 0;

  STORMEIGEN_DEVICE_FUNC virtual const Scalar coeff(DenseIndex index) const = 0;
  STORMEIGEN_DEVICE_FUNC virtual Scalar& coeffRef(DenseIndex index) = 0;
...
class TensorLazyEvaluatorReadOnly : public TensorLazyBaseEvaluator<Dimensions, typename TensorEvaluator<Expr, Device>::Scalar> {
 public:
  ...
  STORMEIGEN_DEVICE_FUNC virtual const Dimensions& dimensions() const {
    return m_dims;
  }
  STORMEIGEN_DEVICE_FUNC virtual const Scalar* data() const {
    return m_impl.data();
  }

  STORMEIGEN_DEVICE_FUNC virtual const Scalar coeff(DenseIndex index) const {
    return m_impl.coeff(index);
  }
  STORMEIGEN_DEVICE_FUNC virtual Scalar& coeffRef(DenseIndex /*index*/) {
...
template<typename PlainObjectType> class TensorRef : public TensorBase<TensorRef<PlainObjectType> >
{
  public:
    ...
    STORMEIGEN_STRONG_INLINE TensorRef() : m_evaluator(NULL) {
    }

    template <typename Expression>
    STORMEIGEN_STRONG_INLINE TensorRef(const Expression& expr) : m_evaluator(new internal::TensorLazyEvaluator<Dimensions, Expression, DefaultDevice>(expr, DefaultDevice())) {
      m_evaluator->incrRefCount();
    }

    template <typename Expression>
    STORMEIGEN_STRONG_INLINE TensorRef& operator = (const Expression& expr) {
      unrefEvaluator();
    ...
    STORMEIGEN_STRONG_INLINE Index rank() const { return m_evaluator->dimensions().size(); }
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Index dimension(Index n) const { return m_evaluator->dimensions()[n]; }
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE const Dimensions& dimensions() const { return m_evaluator->dimensions(); }
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Index size() const { return m_evaluator->dimensions().TotalSize(); }
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE const Scalar* data() const { return m_evaluator->data(); }
    ...
    template<typename... IndexTypes> STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Scalar& coeffRef(Index firstIndex, IndexTypes... otherIndices)
    {
      const std::size_t num_indices = (sizeof...(otherIndices) + 1);
      const array<Index, num_indices> indices{{firstIndex, otherIndices...}};
    ...
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Scalar& coeffRef(Index i0, Index i1)
    {
      array<Index, 2> indices;
      indices[0] = i0;
      indices[1] = i1;
    ...
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Scalar& coeffRef(Index i0, Index i1, Index i2)
    {
      array<Index, 3> indices;
      indices[0] = i0;
      indices[1] = i1;
    ...
    STORMEIGEN_DEVICE_FUNC
    STORMEIGEN_STRONG_INLINE Scalar& coeffRef(Index i0, Index i1, Index i2, Index i3, Index i4)
    {
      array<Index, 5> indices;
      indices[0] = i0;
      indices[1] = i1;
    ...
    template <std::size_t NumIndices> STORMEIGEN_DEVICE_FUNC
  ...
  private:
    STORMEIGEN_STRONG_INLINE void unrefEvaluator() {
      if (m_evaluator) {
...
struct TensorEvaluator<const TensorRef<Derived>, Device>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorEvaluator(const TensorRef<Derived>& m, const Device&)
      : m_ref(m)
  { }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE const Dimensions& dimensions() const { return m_ref.dimensions(); }
  ...
  STORMEIGEN_DEVICE_FUNC Scalar* data() const { return m_ref.data(); }
...
struct TensorEvaluator<TensorRef<Derived>, Device> : public TensorEvaluator<const TensorRef<Derived>, Device>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorEvaluator(TensorRef<Derived>& m, const Device& d) : Base(m, d)
  { }
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_REF_H
