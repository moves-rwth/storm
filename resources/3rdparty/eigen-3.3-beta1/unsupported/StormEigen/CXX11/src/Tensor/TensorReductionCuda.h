...
__device__ STORMEIGEN_ALWAYS_INLINE void atomicReduce(T* output, T accum, R& reducer) {
...
struct FullReducer<Self, Op, GpuDevice, Vectorizable> {
  ...
  STORMEIGEN_DEVICE_FUNC static void run(const Self& self, Op& reducer, const GpuDevice& device, OutputType* output) {
    assert(false && "Should only be called on floats");
  }

  STORMEIGEN_DEVICE_FUNC static void run(const Self& self, Op& reducer, const GpuDevice& device, float* output) {
