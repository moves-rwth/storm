...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_INTDIV_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_INTDIV_H
...
namespace {
  ...
  template <typename T>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE int count_leading_zeros(const T val)
  {
#ifdef __CUDA_ARCH__
  ...
  template <typename T>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE uint32_t muluh(const uint32_t a, const T b) {
  ...
  template <typename T>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE uint64_t muluh(const uint64_t a, const T b) {
#if defined(__CUDA_ARCH__)
  ...
  struct DividerHelper {
    static STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE uint32_t computeMultiplier(const int log_div, const T divider) {
      STORMEIGEN_STATIC_ASSERT(N == 32, YOU_MADE_A_PROGRAMMING_MISTAKE);
  ...
  struct DividerHelper<64, T> {
    static STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE uint64_t computeMultiplier(const int log_div, const T divider) {
...
struct TensorIntDivisor {
 public:
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorIntDivisor() {
    multiplier = 0;
    shift1 = 0;
    shift2 = 0;
  }
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorIntDivisor(const T divider) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T divide(const T numerator) const {
...
class TensorIntDivisor<int32_t, true> {
 public:
  ...
  // Must have 2 <= divider
  STORMEIGEN_DEVICE_FUNC TensorIntDivisor(int32_t divider)  {
    eigen_assert(divider >= 2);
    calcMagic(divider);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE int divide(const int32_t n) const {
#ifdef __CUDA_ARCH__
...
private:
  ...
  // depth explanation.
  STORMEIGEN_DEVICE_FUNC void calcMagic(int32_t d) {
...
static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T operator / (const T& numerator, const TensorIntDivisor<T, div_gt_one>& divisor) {
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_INTDIV_H
