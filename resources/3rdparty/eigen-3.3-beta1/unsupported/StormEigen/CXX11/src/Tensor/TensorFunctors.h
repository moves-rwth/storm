...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_FUNCTORS_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_FUNCTORS_H
...
struct scalar_mod_op {
  STORMEIGEN_DEVICE_FUNC scalar_mod_op(const Scalar& divisor) : m_divisor(divisor) {}
  STORMEIGEN_DEVICE_FUNC inline Scalar operator() (const Scalar& a) const { return a % m_divisor; }
...
struct scalar_sigmoid_op {
  STORMEIGEN_EMPTY_STRUCT_CTOR(scalar_sigmoid_op)
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T operator()(const T& x) const {
    const T one = T(1);
    return one / (one + std::exp(-x));
  }

  template <typename Packet> STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE
...
template <typename T> struct SumReducer
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const T t, T* accum) const {
    (*accum) += t;
  }
  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reducePacket(const Packet& p, Packet* accum) const {
    (*accum) = padd<Packet>(*accum, p);
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T initialize() const {
    return static_cast<T>(0);
  }
  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE Packet initializePacket() const {
    return pset1<Packet>(0);
  }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T finalize(const T accum) const {
    return accum;
  }
  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE Packet finalizePacket(const Packet& vaccum) const {
    return vaccum;
  }
  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T finalizeBoth(const T saccum, const Packet& vaccum) const {
...
template <typename T> struct MeanReducer
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const T t, T* accum) {
    (*accum) += t;
    scalarCount_++;
  }
  template <typename Packet>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reducePacket(const Packet& p, Packet* accum) {
    (*accum) = padd<Packet>(*accum, p);
    packetCount_++;
  }
...
struct AndReducer
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(bool t, bool* accum) const {
    *accum = *accum && t;
  }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE bool initialize() const {
    return true;
  }
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE bool finalize(bool accum) const {
    return accum;
  }
};
...
#if !defined (STORMEIGEN_USE_GPU) || !defined(__CUDACC__) || !defined(__CUDA_ARCH__)
...
template <typename T> class UniformRandomGenerator {
 ...
 public:
  ...
  typename internal::packet_traits<T>::type packetOp(Index, Index = 0) const {
    ...
    STORMEIGEN_ALIGN_MAX T values[packetSize];
...
template <> class UniformRandomGenerator<float> {
 public:
  ...
  typename internal::packet_traits<float>::type packetOp(Index i, Index j = 0) const {
    ...
    STORMEIGEN_ALIGN_MAX float values[packetSize];
...
template <> class UniformRandomGenerator<double> {
 public:
  ...
  typename internal::packet_traits<double>::type packetOp(Index i, Index j = 0) const {
    ...
    STORMEIGEN_ALIGN_MAX double values[packetSize];
    for (int k = 0; k < packetSize; ++k) {
      values[k] = this->operator()(i, j);
    }
...
#if (!defined (STORMEIGEN_USE_GPU) || !defined(__CUDACC__) || !defined(__CUDA_ARCH__)) && __cplusplus > 199711
// We're not compiling a cuda kernel
...
#elif defined (STORMEIGEN_USE_GPU) && defined(__CUDACC__) && defined(__CUDA_ARCH__)
...
class GaussianGenerator {
 public:
  ...
  STORMEIGEN_DEVICE_FUNC GaussianGenerator(const array<T, NumDims>& means,
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_FUNCTORS_H
