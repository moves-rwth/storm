...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_FFT_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_FFT_H
...
class TensorFFTOp : public TensorBase<TensorFFTOp<FFT, XprType, FFTResultType, FFTDir>, ReadOnlyAccessors> {
 public:
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorFFTOp(const XprType& expr, const FFT& fft)
...
struct TensorEvaluator<const TensorFFTOp<FFT, ArgType, FFTResultType, FFTDir>, Device> {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorEvaluator(const XprType& op, const Device& device) : m_fft(op.fft()), m_impl(op.expression(), device), m_data(NULL), m_device(device) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE const Dimensions& dimensions() const {
    return m_dimensions;
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(OutputScalar* data) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE CoeffReturnType coeff(Index index) const {
    return m_data[index];
  }

  template<int LoadMode>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_ALWAYS_INLINE PacketReturnType packet(Index index) const {
 ...
 private:
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void evalToBuf(OutputScalar* data) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE static bool isPowerOfTwo(Index x) {
    eigen_assert(x > 0);
    return !(x & (x - 1));
  }

  // The composite number for padding, used in Bluestein's FFT algorithm
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE static Index findGoodComposite(Index n) {
    Index i = 2;
    while (i < 2 * n - 1) i *= 2;
    return i;
  }

  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE static Index getLog2(Index m) {
    Index log2m = 0;
    while (m >>= 1) log2m++;
    return log2m;
  }
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void processDataLineCooleyTukey(ComplexScalar* line_buf, Index line_len, Index log_len) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void processDataLineBluestein(ComplexScalar* line_buf, Index line_len, Index good_composite, Index log_len, ComplexScalar* a, ComplexScalar* b, const ComplexScalar* pos_j_base_powered) {
    Index n = line_len;
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE static void scramble_FFT(ComplexScalar* data, Index n) {
  ...
  template<int Dir>
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void compute_1D_Butterfly(ComplexScalar* data, Index n, Index n_power_of_2) {
    eigen_assert(isPowerOfTwo(n));
    if (n == 1) {
      return;
    }
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE Index getBaseOffsetFromIndex(Index index, Index omitted_dim) const {
    Index result = 0;
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE Index getIndexFromOffset(Index base, Index omitted_dim, Index offset) const {
...
#endif  // STORMEIGEN_CXX11_TENSOR_TENSOR_FFT_H
