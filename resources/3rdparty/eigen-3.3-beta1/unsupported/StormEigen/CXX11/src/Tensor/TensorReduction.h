...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_REDUCTION_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_REDUCTION_H
...
template <typename OutputDims> struct DimInitializer {
  template <typename InputDims, typename ReducedDims> STORMEIGEN_DEVICE_FUNC
...
template <> struct DimInitializer<Sizes<> > {
  template <typename InputDims, typename Index, size_t Rank> STORMEIGEN_DEVICE_FUNC
...
#if defined(STORMEIGEN_HAS_CONSTEXPR) && defined(STORMEIGEN_HAS_VARIADIC_TEMPLATES)
...
struct GenericDimReducer {
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const Self& self, typename Self::Index firstIndex, Op& reducer, typename Self::CoeffReturnType* accum) {
    STORMEIGEN_STATIC_ASSERT(DimIndex > 0, YOU_MADE_A_PROGRAMMING_MISTAKE);
...
struct GenericDimReducer<-1, Self, Op> {
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const Self& self, typename Self::Index index, Op& reducer, typename Self::CoeffReturnType* accum) {
...
struct InnerMostDimReducer {
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE typename Self::CoeffReturnType reduce(const Self& self, typename Self::Index firstIndex, typename Self::Index numValuesToReduce, Op& reducer) {
...
struct InnerMostDimPreserver {
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const Self&, typename Self::Index, Op&, typename Self::PacketReturnType*) {
...
struct InnerMostDimPreserver<DimIndex, Self, Op, true> {
  static STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE void reduce(const Self& self, typename Self::Index firstIndex, Op& reducer, typename Self::PacketReturnType* accum) {
    STORMEIGEN_STATIC_ASSERT(DimIndex > 0, YOU_MADE_A_PROGRAMMING_MISTAKE);
...
struct FullReducer {
  ...
  static STORMEIGEN_DEVICE_FUNC void run(const Self& self, Op& reducer, const Device&, typename Self::CoeffReturnType* output) {
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType>, Device>
{
  ...
  {
    STORMEIGEN_STATIC_ASSERT(NumInputDims >= NumReducedDims, YOU_MADE_A_PROGRAMMING_MISTAKE);
    STORMEIGEN_STATIC_ASSERT((!ReducingInnerMostDims | !PreservingInnerMostDims | (NumReducedDims == NumInputDims)),
  ...
  STORMEIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(CoeffReturnType* data) {
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE Index firstInput(Index index) const {
    if (ReducingInnerMostDims) {
      if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_REDUCTION_H
