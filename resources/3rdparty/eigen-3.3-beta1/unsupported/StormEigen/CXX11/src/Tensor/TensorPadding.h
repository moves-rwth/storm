...
#ifndef STORMEIGEN_CXX11_TENSOR_TENSOR_PADDING_H
#define STORMEIGEN_CXX11_TENSOR_TENSOR_PADDING_H
...
class TensorPaddingOp : public TensorBase<TensorPaddingOp<PaddingDimensions, XprType>, ReadOnlyAccessors>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE TensorPaddingOp(const XprType& expr, const PaddingDimensions& padding_dims)
...
struct TensorEvaluator<const TensorPaddingOp<PaddingDimensions, ArgType>, Device>
{
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE CoeffReturnType coeff(const array<Index, NumDims>& coords) const
  {
    Index inputIndex;
    if (static_cast<int>(Layout) == static_cast<int>(ColMajor)) {
      {
 ...
 protected:
  ...
  STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE PacketReturnType packetWithPossibleZero(Index index) const
  {
    const int packetSize = internal::unpacket_traits<PacketReturnType>::size;
...
#endif // STORMEIGEN_CXX11_TENSOR_TENSOR_PADDING_H
