...
#ifndef STORMEIGEN_CXX11_TENSORSYMMETRY_TEMPLATEGROUPTHEORY_H
#define STORMEIGEN_CXX11_TENSORSYMMETRY_TEMPLATEGROUPTHEORY_H
...
template<
  template<typename, typename> class Equality,
  typename id
  STORMEIGEN_TPL_PP_SPEC_HACK_DEFC(typename, ts)
>
struct strip_identities<Equality, id, type_list<STORMEIGEN_TPL_PP_SPEC_HACK_USE(ts)>>
{
...
template<
  ...
  int sub_group_size
>
struct dimino_add_cosets_for_rep<Multiply, Equality, id, sub_group_elements, elements, type_list<STORMEIGEN_TPL_PP_SPEC_HACK_USE(empty)>, rep_element, sub_group_size>
{
  typedef elements type;
  constexpr static int global_flags = 0;
};
...
#endif // STORMEIGEN_CXX11_TENSORSYMMETRY_TEMPLATEGROUPTHEORY_H
