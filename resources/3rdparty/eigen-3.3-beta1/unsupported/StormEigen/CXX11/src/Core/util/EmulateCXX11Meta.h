...
#define STORMEIGEN_EMULATE_CXX11_META_H
...
template<std::size_t I, class Head, class Tail>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE typename Head::type array_get(type_list<Head, Tail>&) {
  return get<I, type_list<Head, Tail> >::value;
}
template<std::size_t I, class Head, class Tail>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE typename Head::type array_get(const type_list<Head, Tail>&) {
  return get<I, type_list<Head, Tail> >::value;
}

template <class NList>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE typename NList::HeadType::type array_prod(const NList&) {
  return arg_prod<NList>::value;
}

template<typename t, std::size_t n>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE t array_prod(const array<t, n>& a) {
  t prod = 1;
  for (size_t i = 0; i < n; ++i) { prod *= a[i]; }
  return prod;
}
template<typename t>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE t array_prod(const array<t, 0>& /*a*/) {
  return 0;
}
...
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE T& array_get(std::vector<T>& a) {
  return a[I];
}
template<std::size_t I, class T>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE const T& array_get(const std::vector<T>& a) {
  return a[I];
}
...
struct ArrayApplyAndReduce {
  static inline bool run(const array<A, N>& a) {
    STORMEIGEN_STATIC_ASSERT(N >= 2, YOU_MADE_A_PROGRAMMING_MISTAKE);
...
#endif  // STORMEIGEN_EMULATE_CXX11_META_H
