...
#ifndef STORMEIGEN_CXX11META_H
#define STORMEIGEN_CXX11META_H
...
template<int n STORMEIGEN_TPL_PP_SPEC_HACK_DEFC(typename, as)> struct get<n, type_list<STORMEIGEN_TPL_PP_SPEC_HACK_USE(as)>> { static_assert((n - n) < 0, "meta-template get: The element to extract from a list must be smaller than the size of the list."); };

template<typename T, int n, T a, T... as>                        struct get<n, numeric_list<T, a, as...>>   : get<n-1, numeric_list<T, as...>> {};
template<typename T, T a, T... as>                               struct get<0, numeric_list<T, a, as...>>   { constexpr static T value = a; };
template<typename T, int n STORMEIGEN_TPL_PP_SPEC_HACK_DEFC(T, as)>   struct get<n, numeric_list<T STORMEIGEN_TPL_PP_SPEC_HACK_USEC(as)>> { static_assert((n - n) < 0, "meta-template get: The element to extract from a list must be smaller than the size of the list."); };
...
template<
  template<typename, typename> class test,
  typename check_against
  STORMEIGEN_TPL_PP_SPEC_HACK_DEFC(typename, empty)
>
struct contained_in_list<test, check_against, type_list<STORMEIGEN_TPL_PP_SPEC_HACK_USE(empty)>, false> { constexpr static bool value = false; };
...
template<
  ...
  int last_check_flags
>
struct contained_in_list_gf<test, check_against, type_list<STORMEIGEN_TPL_PP_SPEC_HACK_USE(empty)>, default_flags, false, last_check_flags> { constexpr static bool value = false; constexpr static int global_flags = default_flags; };
...
struct sum_op           {
  template<typename A, typename B> STORMEIGEN_DEVICE_FUNC constexpr static inline auto run(A a, B b) -> decltype(a + b)   { return a + b;   }
...
struct h_array_reduce {
  STORMEIGEN_DEVICE_FUNC constexpr static inline auto run(array<T, N> arr, T identity) -> decltype(Reducer::run(h_array_reduce<Reducer, T, N, n - 1>::run(arr, identity), array_get<n>(arr)))
  {
...
{
  STORMEIGEN_DEVICE_FUNC constexpr static inline T run(const array<T, N>& arr, T)
  {
...
{
  STORMEIGEN_DEVICE_FUNC constexpr static inline T run(const array<T, 0>&, T identity)
  {
    return identity;
  }
};
...
template<typename Reducer, typename T, std::size_t N>
STORMEIGEN_DEVICE_FUNC constexpr inline auto array_reduce(const array<T, N>& arr, T identity) -> decltype(h_array_reduce<Reducer, T, N>::run(arr, identity))
{
...
template<typename T, std::size_t N>
STORMEIGEN_DEVICE_FUNC constexpr inline auto array_sum(const array<T, N>& arr) -> decltype(array_reduce<sum_op, T, N>(arr, static_cast<T>(0)))
{
  return array_reduce<sum_op, T, N>(arr, static_cast<T>(0));
}

template<typename T, std::size_t N>
STORMEIGEN_DEVICE_FUNC constexpr inline auto array_prod(const array<T, N>& arr) -> decltype(array_reduce<product_op, T, N>(arr, static_cast<T>(1)))
{
  return array_reduce<product_op, T, N>(arr, static_cast<T>(1));
}

template<typename t>
STORMEIGEN_DEVICE_FUNC STORMEIGEN_STRONG_INLINE t array_prod(const std::vector<t>& a) {
