...
#ifndef STORMEIGEN_SKYLINEMATRIX_H
#define STORMEIGEN_SKYLINEMATRIX_H
...
public:
    STORMEIGEN_SKYLINE_GENERIC_PUBLIC_INTERFACE(SkylineMatrix)
    STORMEIGEN_SKYLINE_INHERIT_ASSIGNMENT_OPERATOR(SkylineMatrix, +=)
    STORMEIGEN_SKYLINE_INHERIT_ASSIGNMENT_OPERATOR(SkylineMatrix, -=)
...
public:
     ...
     */
    STORMEIGEN_DONT_INLINE Scalar & insert(Index row, Index col) {
        const Index outer = IsRowMajor ? row : col;
        const Index inner = IsRowMajor ? col : row;
    ...
    inline void swap(SkylineMatrix & other) {
        //STORMEIGEN_DBG_SKYLINE(std::cout << "SkylineMatrix:: swap\n");
...
#endif // STORMEIGEN_SkylineMatrix_H
