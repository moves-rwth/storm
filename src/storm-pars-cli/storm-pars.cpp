...
struct SampleInformation {
    ...
    std::vector<
        std::map<typename utility::parametric::VariableType<ValueType>::type, std::vector<typename utility::parametric::CoefficientType<ValueType>::type>>>
        cartesianProducts;
...
std::shared_ptr<storm::models::ModelBase> eliminateScc(std::shared_ptr<storm::models::ModelBase> const& model) {
    ...
    if (model->isOfType(storm::models::ModelType::Dtmc)) {
        ...
        if (model->hasUniqueRewardModel()) {
            ...
            rewardModels.insert(std::make_pair(model->getUniqueRewardModelName(),
                                               storm::models::sparse::StandardRewardModel<ValueType>(boost::none, std::move(actionRewards))));
        }

        result = std::make_shared<storm::models::sparse::Dtmc<ValueType>>(
            std::move(newTransitionMatrix), sparseModel->getStateLabeling().getSubLabeling(selectedStates), std::move(rewardModels));
...
std::shared_ptr<storm::models::ModelBase> simplifyModel(std::shared_ptr<storm::models::ModelBase> const& model, SymbolicInput const& input,
                                                        bool keepRewardsAsConstantAsPossible = false) {
...
template<typename ValueType>
std::shared_ptr<storm::models::ModelBase> removeDontCareChoices(std::shared_ptr<storm::models::ModelBase> const& model, SymbolicInput const& input,
                                                                bool keepRewardsAsConstantAsPossible = false) {
    storm::utility::Stopwatch simplifyingWatch(true);
    std::shared_ptr<storm::models::ModelBase> result;
    if (model->isOfType(storm::models::ModelType::Dtmc)) {
...
PreprocessResult preprocessSparseModel(std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, SymbolicInput const& input,
    ...
    if (regionSettings.isExtremumSet() || monSettings.isMonotonicityAnalysisSet() || parametricSettings.isUseMonotonicitySet() ||
        derSettings.isFeasibleInstantiationSearchSet() || derSettings.getDerivativeAtInstantiation()) {
        ...
        result.model = storm::pars::simplifyModel<ValueType>(result.model, input,
                                                             (monSettings.isMonotonicityAnalysisSet() || parametricSettings.isUseMonotonicitySet()) &&
                                                                 input.properties.begin()->getRawFormula()->isRewardOperatorFormula());
        result.changed = true;
    }
    ...
    if (monSettings.isMonotonicityAnalysisSet() || parametricSettings.isUseMonotonicitySet() || derSettings.isFeasibleInstantiationSearchSet() ||
        derSettings.getDerivativeAtInstantiation()) {
        STORM_LOG_THROW(!input.properties.empty(), storm::exceptions::InvalidSettingsException, "When computing monotonicity, a property has to be specified");
        result.model = storm::pars::removeDontCareChoices<ValueType>(result.model, input,
                                                                     (monSettings.isMonotonicityAnalysisSet() || parametricSettings.isUseMonotonicitySet()) &&
                                                                         input.properties.begin()->getRawFormula()->isRewardOperatorFormula());
        result.changed = true;
        if (mpi.applyBisimulation) {
...
template<typename ValueType>
void printInitialStatesResult(std::unique_ptr<storm::modelchecker::CheckResult> const& result, utility::Stopwatch* watch = nullptr,
                              const utility::parametric::Valuation<ValueType>* valuation = nullptr) {
    if (result) {
...
void verifyPropertiesAtSamplePoints(std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, SymbolicInput const& input,
...
void performGradientDescent(std::shared_ptr<storm::models::sparse::Model<ValueType>> model, SymbolicInput const& input,
    ...
    STORM_LOG_THROW(model->isOfType(storm::models::ModelType::Dtmc), storm::exceptions::NotSupportedException,
                    "Derivative currently only supported for DTMCs.");
    ...
    if (startPointAsString) {
        ...
        std::map<typename utility::parametric::VariableType<ValueType>::type, std::vector<typename utility::parametric::CoefficientType<ValueType>::type>>
            cartesianProduct = samples.cartesianProducts[0];
    ...
    if (auto instantiationString = derSettings.getDerivativeAtInstantiation()) {
        ...
        if (!referenceCheckTask.isRewardModelSet()) {
            ...
            formulaWithoutBound =
                std::make_shared<storm::logic::RewardOperatorFormula>(formulas[0]->asRewardOperatorFormula().getSubformula().asSharedPointer());
        }
        ...
        storm::derivative::GradientDescentInstantiationSearcher<storm::RationalFunction, double> derivativeChecker(
...
template<typename ValueType>
void analyzeMonotonicity(std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, SymbolicInput const& input,
    ...
    if (!monSettings.isMonSolutionSet()) {
        ...
        for (auto& property : input.properties) {
            std::cout << property.getRawFormula()->toString() << std::endl;
...
template<typename ValueType>
void computeRegionExtremumWithSparseEngine(
    ...
    boost::optional<std::pair<std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>,
                              std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>>>& monotoneParameters = boost::none) {
    ...
    for (auto const& property : input.properties) {
        STORM_LOG_THROW(property.getRawFormula()->hasQuantitativeResult(), storm::exceptions::NotSupportedException,
                        "Extremum cannot be computed for this type of property.");
        for (auto const& region : regions) {
            if (monotonicitySettings.useMonotonicity) {
                STORM_PRINT_AND_LOG("Computing extremal value for property " << property.getName() << ": " << *property.getRawFormula() << " within region "
                                                                             << region << " and using monotonicity ..." << std::endl);
            ...
            if (regionSettings.isExtremumSuggestionSet()) {
                ...
                if (storm::api::checkExtremalValue<ValueType>(model, storm::api::createTask<ValueType>(property.getRawFormula(), true), region, engine,
                                                              direction, precision, regionSettings.isAbsolutePrecisionSet(), suggestion, monotonicitySettings,
                ...
                auto valueValuation = storm::api::computeExtremalValue<ValueType>(model, storm::api::createTask<ValueType>(property.getRawFormula(), true),
                                                                                  region, engine, direction, precision, regionSettings.isAbsolutePrecisionSet(),
                                                                                  monotonicitySettings, generateSplitEstimates, monotoneParameters);
                watch.stop();
                ...
                STORM_PRINT_AND_LOG("Result at initial state: " << valueValuation.first << " ( approx. "
                                                                << storm::utility::convertNumber<double>(valueValuation.first) << ") at [" << valuationStr.str()
                                                                << "].\n")
                STORM_PRINT_AND_LOG("Time for model checking: " << watch << ".\n");
            }
        }
...
template<typename ValueType>
void verifyRegionsWithSparseEngine(std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, SymbolicInput const& input,
                                   std::vector<storm::storage::ParameterRegion<ValueType>> const& regions,
    ...
    if (regionSettings.isRefineSet()) {
        ...
        STORM_PRINT_AND_LOG(" with iterative refinement until "
                            << (1.0 - regionSettings.getCoverageThreshold()) * 100.0 << "% is covered."
                            << (regionSettings.isDepthLimitSet() ? " Depth limit is " + std::to_string(regionSettings.getDepthLimit()) + "." : "") << '\n');
        verificationCallback = [&](std::shared_ptr<storm::logic::Formula const> const& formula) {
            ...
            std::unique_ptr<storm::modelchecker::RegionRefinementCheckResult<ValueType>> result = storm::api::checkAndRefineRegionWithSparseEngine<ValueType>(
                model, storm::api::createTask<ValueType>(formula, true), regions.front(), engine, refinementThreshold, optionalDepthLimit,
        ...
        STORM_PRINT_AND_LOG(".\n");
        verificationCallback = [&](std::shared_ptr<storm::logic::Formula const> const& formula) {
            std::unique_ptr<storm::modelchecker::CheckResult> result = storm::api::checkRegionsWithSparseEngine<ValueType>(
                model, storm::api::createTask<ValueType>(formula, true), regions, engine, regionSettings.getHypothesis());
            return result;
        };
    }
...
void verifyWithSparseEngine(
    std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, SymbolicInput const& input,
    std::vector<storm::storage::ParameterRegion<ValueType>> const& regions, SampleInformation<ValueType> const& samples,
    storm::api::MonotonicitySetting monotonicitySettings = storm::api::MonotonicitySetting(),
    boost::optional<std::pair<std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>,
                              std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>>>& monotoneParameters = boost::none,
    uint64_t monThresh = 0, boost::optional<std::set<RationalFunctionVariable>> omittedParameters = boost::none) {
    auto derSettings = storm::settings::getModule<storm::settings::modules::DerivativeSettings>();
...
template<storm::dd::DdType DdType, typename ValueType>
void verifyPropertiesWithSymbolicEngine(std::shared_ptr<storm::models::symbolic::Model<DdType, ValueType>> const& model, SymbolicInput const& input,
                                        SampleInformation<ValueType> const& samples) {
    if (samples.empty()) {
        verifyProperties<ValueType>(
            input.properties,
...
template<storm::dd::DdType DdType, typename ValueType>
void verifyWithDdEngine(std::shared_ptr<storm::models::symbolic::Model<DdType, ValueType>> const& model, SymbolicInput const& input,
                        std::vector<storm::storage::ParameterRegion<ValueType>> const& regions, SampleInformation<ValueType> const& samples) {
    if (regions.empty()) {
...
template<storm::dd::DdType DdType, typename ValueType>
void verifyParametricModel(
    std::shared_ptr<storm::models::ModelBase> const& model, SymbolicInput const& input, std::vector<storm::storage::ParameterRegion<ValueType>> const& regions,
    SampleInformation<ValueType> const& samples, storm::api::MonotonicitySetting monotonicitySettings = storm::api::MonotonicitySetting(),
    boost::optional<std::pair<std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>,
                              std::set<typename storm::storage::ParameterRegion<ValueType>::VariableType>>>& monotoneParameters = boost::none,
    uint64_t monThresh = 0, boost::optional<std::set<RationalFunctionVariable>> omittedParameters = boost::none) {
    if (model->isSparseModel()) {
        storm::pars::verifyWithSparseEngine<ValueType>(model->as<storm::models::sparse::Model<ValueType>>(), input, regions, samples, monotonicitySettings,
                                                       monotoneParameters, monThresh, omittedParameters);
    } else {
...
void processInputWithValueTypeAndDdlib(SymbolicInput& input, storm::cli::ModelProcessingInformation const& mpi) {
    ...
    if (parSettings.onlyObtainConstraints()) {
        ...
        storm::api::exportParametricResultToFile<ValueType>(
            boost::none, storm::analysis::ConstraintCollector<ValueType>(*(model->as<storm::models::sparse::Model<ValueType>>())),
    ...
    if (model) {
        ...
        if (monSettings.isMonotoneParametersSet()) {
            monotoneParameters = std::move(storm::api::parseMonotoneParameters<ValueType>(monSettings.getMonotoneParameterFilename(),
                                                                                          model->as<storm::models::sparse::Model<ValueType>>()));
        }
        // TODO: is onlyGlobalSet was used here
        verifyParametricModel<DdType, ValueType>(
            model, input, regions, samples,
            storm::api::MonotonicitySetting(parSettings.isUseMonotonicitySet(), false, monSettings.isUsePLABoundsSet(), monSettings.isDisableOptimizationSet()),
