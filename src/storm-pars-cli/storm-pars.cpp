...
namespace storm {
    namespace pars {
        ...
        void parameterSpacePartitioningWithSparseEngine(std::shared_ptr<storm::models::sparse::Model<ValueType>> const& model, cli::SymbolicInput const& input, std::vector<storm::storage::ParameterRegion<ValueType>> const& regions, storm::api::MonotonicitySetting monotonicitySettings = storm::api::MonotonicitySetting(), uint64_t monThresh = 0) {
            ...
            STORM_PRINT_AND_LOG(" with iterative refinement until " << (1.0 - partitionSettings.getCoverageThreshold()) * 100.0 << "% is covered." << (partitionSettings.isDepthLimitSet() ? " Depth limit is " + std::to_string(partitionSettings.getDepthLimit()) + "." : "") << '\n');

            storm::cli::printModelCheckingProperty(property);
            storm::utility::Stopwatch watch(true);
            std::unique_ptr<storm::modelchecker::CheckResult> result =  storm::api::checkAndRefineRegionWithSparseEngine<ValueType>(model, storm::api::createTask<ValueType>((property.getRawFormula()), true), regions.front(), engine, refinementThreshold, optionalDepthLimit, storm::modelchecker::RegionResultHypothesis::Unknown, false, monotonicitySettings, monThresh);
            watch.stop();
        ...
        void processInputWithValueTypeAndDdlib(cli::SymbolicInput& input, storm::cli::ModelProcessingInformation const& mpi) {
            ...
            if (mode == storm::pars::utility::ParametricMode::SolutionFunction) {
                ...
                STORM_LOG_THROW(regions.empty(), storm::exceptions::InvalidSettingsException, "Solution function computations cannot be restricted to specific regions");
                ...
                std::vector<std::shared_ptr<storm::logic::Formula const>> formulas = storm::api::extractFormulasFromProperties(input.properties);
                STORM_LOG_THROW(formulas.size() == 1,  storm::exceptions::InvalidSettingsException, "Feasibility analysis is only supported for single-objective properties.");
                auto formula = formulas[0];
                storm::pars::performFeasibility<ValueType>(model->as<storm::models::sparse::Model<ValueType>>(),
                                                               createFeasibilitySynthesisTaskFromSettings(formula, regions),
                                                               omittedParameters, monotonicitySettings);
            } else if (mode == storm::pars::utility::ParametricMode::Verification) {
                STORM_LOG_INFO("Verification mode started.");
                STORM_LOG_THROW(input.properties.size() == 1,  storm::exceptions::InvalidSettingsException, "Verification analysis is only supported for single-objective properties.");
                ...
                STORM_LOG_THROW(model->isSparseModel(),  storm::exceptions::InvalidSettingsException, "Parameter space partitioning is only supported on sparse models.");
                ...
                storm::pars::parameterSpacePartitioningWithSparseEngine(model->as<storm::models::sparse::Model<ValueType>>(), input, regions, monotonicitySettings, monThresh);
                ...
                if (!samplesAsString.empty()) {
                    samples = parseSamples<ValueType>(model,
                                                      samplesAsString,
                                                      sampleSettings.isSamplesAreGraphPreservingSet());
                ...
                if (!samples.empty()) {
                    ...
                    if (samples.exact) {
                        verifyPropertiesAtSamplePointsWithSparseEngine<ValueType, storm::RationalNumber>(model->as<storm::models::sparse::Model<ValueType>>(), input, samples);
                    } else {
