...
namespace {
    ...
    public:
        ...
        static storm::Environment createEnvironment() {
            ...
            //env.solver().gmmxx().setPrecision(storm::utility::convertNumber<storm::RationalNumber>(1e-6)); // Need to increase precision because eq sys yields incorrect results
            return env;
        }
    };
    ...
    public:
        ...
        typename std::enable_if<std::is_same<MT, SparseModelType>::value, std::shared_ptr<MT>>::type
        buildJaniModel(std::string const& pathToJaniModel, std::string const& constantDefinitionString = "") const {
    ...
    TYPED_TEST(SteadyStateCtmcCslModelCheckerTest, steadystatetest) {
        ...
        EXPECT_EQ(sortedVector[0], storm::utility::zero<ValueType>()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_EQ(sortedVector[1], storm::utility::zero<ValueType>()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_EQ(sortedVector[2], storm::utility::zero<ValueType>()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_NEAR(sortedVector[3], this->parseNumber("1/35"),this->precision()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_NEAR(sortedVector[4], this->parseNumber("2/35"),this->precision()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_NEAR(sortedVector[5], this->parseNumber("4/35"),this->precision()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_NEAR(sortedVector[6], this->parseNumber("1/5"),this->precision()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
        EXPECT_NEAR(sortedVector[7], this->parseNumber("3/5"),this->precision()) << "Result of steady state computation is " << storm::utility::vector::toString(resultVector) << std::endl;
    }
}
