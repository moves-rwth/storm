...
TEST(ExplicitJitJaniModelBuilderTest, Dtmc) {
    ...
    std::shared_ptr<storm::models::sparse::Model<double>> model = storm::builder::jit::ExplicitJitJaniModelBuilder<double>(janiModel).build().getModel();;
    EXPECT_EQ(13ul, model->getNumberOfStates());
    EXPECT_EQ(20ul, model->getNumberOfTransitions());
    
    program = storm::parser::PrismParser::parse(STORM_TEST_RESOURCES_DIR "/dtmc/brp-16-2.pm");
    janiModel = program.toJani();
    model = storm::builder::jit::ExplicitJitJaniModelBuilder<double>(janiModel).build().getModel();;
...
TEST(ExplicitJitJaniModelBuilderTest, FailComposition) {
    ...
    ASSERT_THROW(storm::builder::jit::ExplicitJitJaniModelBuilder<double>(janiModel).build().getModel();, storm::exceptions::WrongFormatException);
}
