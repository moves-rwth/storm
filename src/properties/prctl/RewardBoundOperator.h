#ifndef STORM_PROPERTIES_PRCTL_REWARDBOUNDOPERATOR_H_
#define STORM_PROPERTIES_PRCTL_REWARDBOUNDOPERATOR_H_
		...
		namespace prctl {
			...
			class IRewardBoundOperatorModelChecker {
				public:
					 ...
					 */
					virtual storm::storage::BitVector checkRewardBoundOperator(RewardBoundOperator<T> const & obj) const = 0;
			};
			...
			public:
				...
				virtual std::shared_ptr<AbstractStateFormula<T>> clone() const override {
					...
					auto result = std::make_shared<RewardBoundOperator<T>>();
...
#endif /* STORM_PROPERTIES_PRCTL_REWARDBOUNDOPERATOR_H_ */
