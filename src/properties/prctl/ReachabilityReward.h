#ifndef STORM_PROPERTIES_PRCTL_REACHABILITYREWARD_H_
#define STORM_PROPERTIES_PRCTL_REACHABILITYREWARD_H_
		...
		namespace prctl {
			...
			class IReachabilityRewardModelChecker {
				public:
					 ...
					 */
					virtual std::vector<T> checkReachabilityReward(ReachabilityReward<T> const & obj, bool qualitative) const = 0;
			};
			...
			public:
				...
				virtual std::shared_ptr<AbstractRewardPathFormula<T>> clone() const override {
					...
					auto result = std::make_shared<ReachabilityReward<T>>();
					if (this->isChildSet()) {
...
#endif /* STORM_PROPERTIES_PRCTL_REACHABILITYREWARD_H_ */
