...
namespace storm {
    namespace analysis {
        ...
        std::pair<std::shared_ptr<expressions::BinaryRelationExpression>, AssumptionStatus> AssumptionMaker<ValueType, ConstantType>::createAndCheckAssumption(uint_fast64_t val1, uint_fast64_t val2, expressions::BinaryRelationExpression::RelationType relationType, std::shared_ptr<Order> order, storage::ParameterRegion<ValueType> region,  std::vector<ConstantType> const minValues, std::vector<ConstantType> const maxValues) const {
            auto strVal1 = std::to_string(val1);
            auto strVal2 = std::to_string(val2);
            expressions::Variable var1;
            if (!expressionManager->hasVariable(strVal1)) {
                var1 = expressionManager->declareRationalVariable(strVal1);
            } else {
                var1 = expressionManager->getVariable(strVal1);
            }
            expressions::Variable var2;
            if (!expressionManager->hasVariable(strVal2)) {
                var1 = expressionManager->declareRationalVariable(strVal2);
            } else {
                var1 = expressionManager->getVariable(strVal2);
            }
            ...
            STORM_LOG_INFO("Assumption created");
            ...
            STORM_LOG_INFO("Assumption validated");
