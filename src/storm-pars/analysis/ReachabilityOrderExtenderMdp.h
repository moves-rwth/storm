    ...
    namespace analysis {
        ...
        class ReachabilityOrderExtenderMdp : public ReachabilityOrderExtender<ValueType, ConstantType> {
            public:
                ...
                ReachabilityOrderExtenderMdp(std::shared_ptr<models::sparse::Model<ValueType>> model, std::shared_ptr<logic::Formula const> formula, bool prMax = true, bool useAssumptions = true);

                ReachabilityOrderExtenderMdp(storm::storage::BitVector* topStates,  storm::storage::BitVector* bottomStates, storm::storage::SparseMatrix<ValueType> matrix, bool prMax = true, bool useAssumptions = true);
