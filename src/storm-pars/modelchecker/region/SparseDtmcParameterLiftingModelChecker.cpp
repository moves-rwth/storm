...
void SparseDtmcParameterLiftingModelChecker<SparseModelType, ConstantType>::specifyUntilFormula(
    ...
    if (RegionModelChecker<ValueType>::isUseMonotonicitySet()) {
        STORM_LOG_THROW(!statesWithProbability01.first.empty(), storm::exceptions::InvalidPropertyException,
                        "There should be state with probability of reaching goal of 0");
        STORM_LOG_THROW(!statesWithProbability01.second.empty(), storm::exceptions::InvalidPropertyException,
                        "There should be state with probability of reaching goal of 1");
        this->orderExtender = std::make_shared<storm::analysis::ReachabilityOrderExtender<ValueType, ConstantType>>(
            statesWithProbability01.second, statesWithProbability01.first, this->parametricModel->getTransitionMatrix());
    }
}
    ...
    if (stepBound) {
        ...
        if (isRegionSplitEstimateSupported()) {
            computeRegionSplitEstimates(x, solver->getSchedulerChoices(), region, dirForParameters,
                                        localMonotonicityResult == nullptr ? nullptr : localMonotonicityResult->getGlobalMonotonicityResult());
        }
    }
...
void SparseDtmcParameterLiftingModelChecker<SparseModelType, ConstantType>::computeRegionSplitEstimates(
    ...
    std::shared_ptr<analysis::MonotonicityResult<VariableType>> monRes) {
...
void SparseDtmcParameterLiftingModelChecker<SparseModelType, ConstantType>::splitSmart(storm::storage::ParameterRegion<ValueType>& region,
    ...
    if (regionSplitEstimationsEnabled && useRegionSplitEstimates) {
        ...
        if (this->isDisableOptimizationSet()) {
            ...
            region.split(region.getSplittingPoint(consideredVariables, this->possibleMonotoneIncrParameters, this->possibleMonotoneDecrParameters, minimize),
                         regionVector, std::move(consideredVariables), this->possibleMonotoneParameters);
        }
    } else {
