...
SparseParameterLiftingModelChecker<SparseModelType, ConstantType>::computeExtremalValue(
    ...
    if (useMonotonicity) {
        if (this->isUseBoundsSet()) {
            ...
            if (minimize) {
                maxBound = getBound(env, region, storm::solver::OptimizationDirection::Maximize, nullptr)
                               ->template asExplicitQuantitativeCheckResult<ConstantType>()
                               .getValueVector();
                minBound = getBound(env, region, storm::solver::OptimizationDirection::Minimize, nullptr)
                               ->template asExplicitQuantitativeCheckResult<ConstantType>()
                               .getValueVector();
        ...
        while (!regionQueue.empty()) {
            ...
            if (investigateBounds) {
                ...
                if (lookAtRegion) {
                    ...
                    //                                     : (useMonotonicity ? currRegion.getPoint(dir,
                    //                                     *(localMonotonicityResult->getGlobalMonotonicityResult()),
                    //                                                                              possibleMonotoneIncrParameters,
                    //                                                                              possibleMonotoneDecrParameters)
                    //                                                        : currRegion.getCenterPoint());

                    auto point = (useMonotonicity ? currRegion.getPoint(dir, *(localMonotonicityResult->getGlobalMonotonicityResult()),
                    ...
                    if (splitRegion) {
                        ...
                        if (useMonotonicity) {
                            this->splitSmart(currRegion, newRegions, *(localMonotonicityResult->getGlobalMonotonicityResult()), minimize);
                        ...
                        if (useMonotonicity && this->isUseBoundsSet() && !order->getDoneBuilding() && orderExtender &&
                            ...
                            if (minimize) {
                                ...
                                orderExtender->setMinMaxValues(
                                    getBound(env, currRegion, storm::solver::OptimizationDirection::Minimize, localMonotonicityResult)
                                        ->template asExplicitQuantitativeCheckResult<ConstantType>()
                                        .getValueVector(),
                                    bounds, order);
                            }
...
SparseParameterLiftingModelChecker<SparseModelType, ConstantType>::computeExtremalValue(
    Environment const& env, storm::storage::ParameterRegion<typename SparseModelType::ValueType> const& region, storm::solver::OptimizationDirection const& dir,
    typename SparseModelType::ValueType const& precision, bool absolutePrecision) {
    return computeExtremalValue(env, region, dir, precision, absolutePrecision, boost::none);
}

template<typename SparseModelType, typename ConstantType>
bool SparseParameterLiftingModelChecker<SparseModelType, ConstantType>::checkExtremalValue(
    Environment const& env, storm::storage::ParameterRegion<typename SparseModelType::ValueType> const& region, storm::solver::OptimizationDirection const& dir,
    typename SparseModelType::ValueType const& precision, bool absolutePrecision, typename SparseModelType::ValueType const& valueToCheck) {
    ...
    return storm::solver::minimize(dir) ? storm::utility::convertNumber<ConstantType>(res) >= storm::utility::convertNumber<ConstantType>(valueToCheck)
...
template<typename SparseModelType, typename ConstantType>
std::shared_ptr<storm::analysis::Order> SparseParameterLiftingModelChecker<SparseModelType, ConstantType>::getInitialOrder(
    ...
    if (this->orderExtender) {
        ...
        std::shared_ptr<storm::analysis::ReachabilityOrderExtender<ValueType, ConstantType>> castedPointerReach =
            std::dynamic_pointer_cast<storm::analysis::ReachabilityOrderExtender<ValueType, ConstantType>>(this->orderExtender);
        if (castedPointerReach != nullptr) {
            res = castedPointerReach->toOrder(region);
        }
        std::shared_ptr<storm::analysis::RewardOrderExtender<ValueType, ConstantType>> castedPointerRew =
            std::dynamic_pointer_cast<storm::analysis::RewardOrderExtender<ValueType, ConstantType>>(this->orderExtender);
