...
void SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::specifyBoundedUntilFormula(
    ...
    if (!maybeStates.empty()) {
        ...
        parameterLifter = std::make_unique<storm::transformer::ParameterLifter<typename SparseModelType::ValueType, ConstantType>>(
            this->parametricModel->getTransitionMatrix(), b, this->parametricModel->getTransitionMatrix().getRowFilter(maybeStates), maybeStates, false,
            this->isUseMonotonicitySet());
    ...
    if (this->isUseMonotonicitySet()) {
        ...
        this->orderExtender = std::make_shared<storm::analysis::ReachabilityOrderExtender<ValueType, ConstantType>>(
            ...
            storm::solver::maximize(checkTask.getOptimizationDirection()));
    }
}
...
void SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::specifyReachabilityRewardFormula(
    ...
    if (!maybeStates.empty()) {
        ...
        parameterLifter = std::make_unique<storm::transformer::ParameterLifter<typename SparseModelType::ValueType, ConstantType>>(
            this->parametricModel->getTransitionMatrix(), b, selectedRows, maybeStates, false, this->isUseMonotonicitySet());
...
void SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::specifyCumulativeRewardFormula(
    ...
    parameterLifter = std::make_unique<storm::transformer::ParameterLifter<typename SparseModelType::ValueType, ConstantType>>(
        ...
        maybeStates, false, this->isUseMonotonicitySet());
    computePlayer1Matrix();
...
std::unique_ptr<CheckResult> SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::computeQuantitativeValues(
    ...
    STORM_LOG_ASSERT(localMonotonicityResult == nullptr, "Not using local monotonicity result for mdps");
...
template<typename SparseModelType, typename ConstantType>
void SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::extendLocalMonotonicityResult(
...
template<typename SparseModelType, typename ConstantType>
void SparseMdpParameterLiftingModelChecker<SparseModelType, ConstantType>::splitSmart(storm::storage::ParameterRegion<ValueType>& region,
