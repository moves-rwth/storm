...
PartitionSettings::PartitionSettings() : ModuleSettings(moduleName) {
    this->addOption(storm::settings::OptionBuilder(moduleName, requestedCoverageOptionName, false, "The requested coverage")
                        .addArgument(storm::settings::ArgumentBuilder::createDoubleArgument("coverage-threshold", "The fraction of unknown area falls below this threshold.").setDefaultValueDouble(0.05).addValidatorDouble(storm::settings::ArgumentValidatorFactory::createDoubleRangeValidatorIncluding(0.0,1.0)).build())
                        .addArgument(storm::settings::ArgumentBuilder::createIntegerArgument("depth-limit", "(advanced) If given, limits the number of times a region is refined.").setDefaultValueInteger(-1).makeOptional().build()).build());
...
bool PartitionSettings::isDepthLimitSet() const {
    return this->getOption(requestedCoverageOptionName).getArgumentByName("depth-limit").getHasBeenSet() && this->getOption(requestedCoverageOptionName).getArgumentByName("depth-limit").getValueAsInteger() >= 0;
}
