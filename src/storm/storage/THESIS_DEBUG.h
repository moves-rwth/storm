...
MecBenchmarkType debugIntToBenchmarkType(uint64_t forcedMecAlgorithm)
{
    switch(forcedMecAlgorithm) {
        case 1 : return MecBenchmarkType::NAIVE;
        case 2 : return MecBenchmarkType::NAIVE_STATS;
        case 3 : return MecBenchmarkType::LOCKSTEP;
        case 4 : return MecBenchmarkType::LOCKSTEP_STATS;
        case 5 : return MecBenchmarkType::INTERLEAVE;
        case 6 : return MecBenchmarkType::INTERLEAVE_STATS;
        default: STORM_LOG_ASSERT(false, "Unexpected symbolic benchmark int");
    }
...
{
    switch(type) {
        case MecBenchmarkType::NAIVE : return "NAIVE";
        case MecBenchmarkType::NAIVE_STATS : return "NAIVE-STATS";
        case MecBenchmarkType::LOCKSTEP : return "LOCKSTEP";
        case MecBenchmarkType::LOCKSTEP_STATS : return "LOCKSTEP-STATS";
        case MecBenchmarkType::INTERLEAVE : return "INTERLEAVE";
        case MecBenchmarkType::INTERLEAVE_STATS : return "INTERLEAVE-STATS";
    }
}
...
template<storm::dd::DdType DdType, typename ValueType>
BenchmarkResult doSymbolicBenchmark(storm::models::symbolic::NondeterministicModel<DdType, ValueType> const& symbolicModel,
                                    MecBenchmarkType type) {
    ...
    std::cout << "Symbolic model stats: " << std::endl
                ...
                << "States: " << symbolicModel.getNumberOfStates() << " (" << symbolicModel.getReachableStates().getNodeCount() << " nodes)"
                << std::endl
                << "Transitions: " << symbolicModel.getNumberOfTransitions() << " (" << symbolicModel.getTransitionMatrix().toBdd().getNodeCount()
                << " nodes)" << std::endl
                << "Choices: " << symbolicModel.getNumberOfChoices() << std::endl
                << "Variables Total: "
                << (symbolicModel.getRowVariables().size() + symbolicModel.getColumnVariables().size() +
                    symbolicModel.getNondeterminismVariables().size())
                ...
                << "Variables Nondeterminism: " << symbolicModel.getNondeterminismVariables().size() << " (" << ddNondeterminismCount
                << " DD variables)" << std::endl
                << std::endl;
...
template<storm::dd::DdType DdType, typename ValueType>
void doMecBenchmark(storm::models::ModelBase const& model,
                    uint64_t forcedMecAlgorithm) {
