...
namespace storm {
    ...
    namespace expressions {
        ...
        boost::any JaniSyntacticalEqualityCheckVisitor::visit(ValueArrayExpression const& expression, boost::any const& data) {
            ...
            auto rhs = std::dynamic_pointer_cast<storm::expressions::ValueArrayExpression>(otherBaseExpression.getSharedPointer());
            if (rhs) {
        ...
        boost::any JaniSyntacticalEqualityCheckVisitor::visit(ConstructorArrayExpression const& expression, boost::any const& data) {
            BaseExpression const& otherBaseExpression = boost::any_cast<std::reference_wrapper<BaseExpression const>>(data).get();
            auto rhs = std::dynamic_pointer_cast<storm::expressions::ConstructorArrayExpression>(otherBaseExpression.getSharedPointer());
            if (rhs) {
                return boost::any_cast<bool>(expression.size()->accept(*this, std::ref(*rhs->size()))) && boost::any_cast<bool>(expression.getElementExpression()->accept(*this, std::ref(*rhs->getElementExpression()))) && (expression.getIndexVar() == rhs->getIndexVar());
            } else {
                return false;
            }
        }
        ...
        boost::any JaniSyntacticalEqualityCheckVisitor::visit(ArrayAccessExpression const& expression, boost::any const& data) {
            BaseExpression const& otherBaseExpression = boost::any_cast<std::reference_wrapper<BaseExpression const>>(data).get();
            auto rhs = std::dynamic_pointer_cast<storm::expressions::ArrayAccessExpression>(otherBaseExpression.getSharedPointer());
            if (rhs) {
                return boost::any_cast<bool>(expression.getFirstOperand()->accept(*this, std::ref(*rhs->getFirstOperand()))) && boost::any_cast<bool>(expression.getSecondOperand()->accept(*this, std::ref(*rhs->getSecondOperand())));
            } else {
                return false;
            }
        }
        ...
        boost::any JaniSyntacticalEqualityCheckVisitor::visit(FunctionCallExpression const& expression, boost::any const& data) {
            BaseExpression const& otherBaseExpression = boost::any_cast<std::reference_wrapper<BaseExpression const>>(data).get();
            auto rhs = std::dynamic_pointer_cast<storm::expressions::FunctionCallExpression>(otherBaseExpression.getSharedPointer());
            if (rhs) {
                if (expression.getFunctionIdentifier() != rhs->getFunctionIdentifier() || expression.getNumberOfArguments() != rhs->getNumberOfArguments()) {
                    return false;
                }
                for (int64_t i = 0; i < expression.getNumberOfArguments(); ++i) {
