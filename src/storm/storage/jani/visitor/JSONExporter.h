...
class FormulaToJaniJson : public storm::logic::FormulaVisitor {
   public:
    ...
    bool containsTradeoffProperties() const;  // Returns true iff the  previously translated formula contained a tradeoff property (multi objective formula)
   ...
   private:
    FormulaToJaniJson(storm::jani::Model const& model) : model(model), stateExitRewards(false), tradeoffProperties(false) {}
    ...
    mutable bool tradeoffProperties;
};
