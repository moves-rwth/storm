        ...
        Expression RationalFunctionToExpression<ValueType>::toExpression(ValueType function) {
            ...
            if (function.isConstant()) {
                ...
                for (auto itr = nominator.begin(); itr != nominator.end(); ++itr) {
                    ...
                    for (auto var : varsFunction) {
                        nominatorPartExpr = nominatorPartExpr * storm::expressions::pow(manager->getVariable(var.name()), manager->rational(storm::utility::convertNumber<storm::RationalNumber>(itr->monomial()->exponentOfVariable(var))));
                    }
