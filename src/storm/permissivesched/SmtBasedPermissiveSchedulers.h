...
namespace storm {
    namespace ps {
        ...
        public:
            ...
            :  PermissiveSchedulerComputation<RM>(mdp, goalstates, sinkstates), mPerformedSmtLoop(false), mFoundSolution(false), solver(smtSolver), manager(solver.getManager()) {
...
//                        }
            ...
            void performSmtLoop(bool lowerBound, double boundary, PermissiveSchedulerPenalties const& penalties) {
                ...
                if (result == storm::solver::SmtSolver::CheckResult::Sat) {
                    ...
                    std::sort(availableStateActionPairs.begin(), availableStateActionPairs.end(), [&penalties] (storage::StateActionPair const& first, storage::StateActionPair const& second) { return penalties.get(first) < penalties.get(second); } );
