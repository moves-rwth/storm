...
namespace storm {
    namespace generator {
        ArrayVariableReplacementInformation::ArrayVariableReplacementInformation(std::vector<ArrayVariableReplacementInformation>&& children) : data(std::move(children)) {
        ...
        uint64_t ArrayVariableReplacementInformation::getVariableInformationIndex(std::vector<uint64_t> arrayIndexVector) const {
            ...
            for (auto const& i : arrayIndexVector) {
                ...
                STORM_LOG_THROW(i < replInfo->getNumberOfChildren(), storm::exceptions::OutOfRangeException, "Array access evaluates to array index " << i << " which is out of bounds as the array size is " << replInfo->getNumberOfChildren());
