...
std::vector<std::tuple<int, double, std::string, int>> storm::bdmp::FigaroProgram::showFireableOccurrences()
{
...
void storm::bdmp::FigaroProgram::runOnceInteractionStep_default_step()
{
	if ( !false )
	{
		floatState[calculated_lambda_OF_Arrival]  =  0.001;
	}

	if ( !false )
	{
		floatState[calculated_lambda_OF_end_of_payment]  =  0.001;
	}

	if ( !false )
	{
		floatState[calculated_lambda_OF_tank_is_full_1]  =  0.001;
	}

	if ( !false )
	{
		floatState[calculated_lambda_OF_tank_is_full_2]  =  0.001;
	}

	if ( !false )
	{
		floatState[calculated_lambda_OF_tank_is_full_3]  =  0.001;
	}
...
void storm::bdmp::FigaroProgram::runInteractions() {
    int counter = 0;
    int comparator = 1;
    doReinitialisations();
